...
%% 优化
% 命令行输入需要执行的优化方法
opt1 = input(' 0 - Optimize (r g K) values according to https://doi.org/10.1016/S0011-2275(02)00035-8\n 1 - Optimize (a R K) values according to https://doi.org/10.1016/S0140-7007(99)00046-8\n Input 0 or 1 to select corresponding method to get the TEC parameters: ');
opt2 = input('TEC设定方式（输入0为电流、1为电压）：');
TE_log(sprintf('Set opt1 = %d；opt2 = %d',opt1,opt2))
% 设定优化向量的初值
switch opt1
    ...
    case(1) % 优化(a R K)值，见参考文献[2]
        opt1a = input('Input polynomial order to correlate the (a R K) values: ');
        TE_log(sprintf('Using %d-order polynomial function to correlate (a R K)', opt1a), 0, 1)
        x0 = ones(3, opt1a+1);
        x0(:,2:(opt1a+1)) = 0;
    otherwise
        prompt = sprintf('Unknown running mode of %d in case_GetTECParams.m', opt1);
        TE_log(prompt, 1);
        return
end
% 定义优化参数
...
% 定义目标函数
fun = @(x)(TE_RMSE(x,TEC,ExpData,opt1,opt2));
% 获得优化参数
